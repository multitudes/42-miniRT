...
/*   Updated: 2024/10/04 16:56:42 by lbrusa           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
						...
						t_scatter_record srec, int depth)
{
	...
	t_color ambient_material = vec3multscalar(vec3add(metal->albedo, \
								vec3multscalar(ambient_light, 0.1)), 0.01);
	t_color reflected_color = vec3mult(srec.attenuation, ray_color(scene, \
								...
								int depth)
{
	...
	params.scattered = ray(params.rec.p, \
						mixture_pdf_generate(&params.mix_pdf));
	params.pdf_value = mixture_pdf_value(&params.mix_pdf, \
											&params.scattered.dir);
	params.scattering_pdf = params.rec.mat->scattering_pdf(params.rec.mat, \
							r, &params.rec, &params.scattered);
