...
/*   Updated: 2024/10/09 10:33:43 by lbrusa           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	cylinder_intersection_check(double t, t_interval *ray_t, t_cyl_utils *utils)
{
	t_vec3 point;
	t_vec3 delta_point;
	double height; 
	...
	if (surrounds(ray_t, t)) {
        ...
        if (utils->cyl->min < height && height < utils->cyl->max) 
		{
            if (utils->closest_t < 0 || t < utils->closest_t) 
			{
                ...
                utils->normal = unit_vector(vec3substr(point, vec3add(utils->cyl->center, \
